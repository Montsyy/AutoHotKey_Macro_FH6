...
<img width="580" height="400" alt="Image" src="https://github.com/user-attachments/assets/66f22110-160d-42f0-a38a-4120e060e62d" />
...
* **Active Step Highlighting:** The table dynamically highlights and auto-scrolls to the current step being executed, so you always know exactly where the macro is in its sequence.
...
* **1 Loop Estimation:** Automatically calculates and displays the exact time it takes to complete one full loop in a clean `MM:SS` (or `MM:SS.ms`) format.
* **Dynamic Hotkeys:** Change the Start/Stop hotkey directly from the UI without editing the code.
* **Safety Locks & Clean UI:** The UI prevents accidental edits or deletions while the macro is actively running to ensure stability. Popups are properly layered so they never hide behind the main window, and a prominent red `Clear` button helps avoid accidental clicks.
...
3. Press your configured hotkey to start the macro. Press it again to stop it at any time. The table will highlight each row as it executes.
